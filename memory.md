...
## Snapshot (2026-06-02)
- **Phase:** Phase 0-4 ✅ COMPLETE. **Phase 5 (replenishment engine) IN PROGRESS** — core formulas done (#49: safety stock / reorder point / EOQ / days-of-cover / order-up-to); service-level→reorder-suggestions and stockout/dead-stock KPIs remain. All code verified on `main`.
...
2. `feat(ml): service-level targets → reorder suggestions` — NEXT. Combine the demand quantiles (`EmpiricalQuantileForecaster`) with the replenishment math: a service level maps to a demand quantile → lead-time-demand quantile → suggested reorder qty (likely a `replenishment/suggestions.py` with a `ReorderSuggestion` dataclass bundling reorder point, order qty/EOQ, days-of-cover, the service level used). This is the payoff of the Phase 3 quantile work.
3. `feat(ml): simulated stockout / dead-stock KPIs` — simulate a policy over history to report stockout rate, dead-stock value, fill rate, turns.
