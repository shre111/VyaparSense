...
## Snapshot (2026-06-02)
- **Phase:** Phase 0-4 ✅ COMPLETE. **Phase 5 (replenishment engine) IN PROGRESS** — core formulas (#49) and service-level reorder suggestions (#51) done; only simulated stockout/dead-stock KPIs remain. All code verified on `main`.
...
## Next actions (Phase 5 — replenishment engine)
Per `docs/backlog.md` Phase 5:
1. ✅ `feat(ml): safety stock + reorder point + EOQ + days-of-cover` (#49, done) — `replenishment/inventory.py`.
2. ✅ `feat(ml): service-level targets → reorder suggestions` (#51, done) — `replenishment/suggestions.py`.
3. `feat(ml): simulated stockout / dead-stock KPIs` — NEXT, last Phase 5 item. Simulate an inventory policy (the `ReorderSuggestion` reorder-point/order-up-to logic) over historical demand to report business KPIs: stockout rate, fill rate, dead-stock value, inventory turns, days-of-supply. Likely `replenishment/simulation.py` with a day-by-day loop (on-hand, on-order with lead-time arrival, reorder when ≤ ROP) → a `SimulationResult` of KPIs. Pure stdlib; feed it the per-day demand + a reorder policy. Metrics per CLAUDE.md §5 business KPIs.
- Cross-cutting (Phase 5/6): wire selection + global model into `POST /tenants/{id}/forecasts` persisting into append-only `forecasts` → enables the accuracy-over-time chart (the flywheel/hero visual). `GlobalLightGBM` is NOT a `Baseline` (fits across series) — integration must handle both the per-series `select_per_series` path and the global path.
