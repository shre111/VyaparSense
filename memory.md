...
## Snapshot (2026-06-02)
- **Phase:** Phase 0-4 ✅ COMPLETE — Phase 4 (global ML model): feature engineering (#43), global LightGBM + global backtest (#45), model cards (#47). All code verified on `main`. **Phase 5 (replenishment engine: safety stock / reorder point / EOQ / service levels) is next.**
...
Phase 4 is fully done. Per `docs/backlog.md` Phase 5:
1. `feat(ml): safety stock + reorder point + EOQ + days-of-cover` — NEXT. Likely a new `replenishment/` subpackage (CLAUDE.md §3 target layout: `packages/ml/.../replenishment/`). Pure formulas: safety stock = z(service_level)·σ_demand·√lead_time; reorder point = mean_demand·lead_time + safety_stock; EOQ = √(2·D·order_cost / holding_cost); days-of-cover = on_hand / mean_daily_demand. Pure stdlib (z-score via statistics.NormalDist).
2. `feat(ml): service-level targets → reorder suggestions` — combine the demand quantiles (`EmpiricalQuantileForecaster`) with the replenishment math; a service level maps to a demand quantile → reorder qty.
3. `feat(ml): simulated stockout / dead-stock KPIs`.
- The probabilistic forecasts (Phase 3) feed service-level-aware reorder points here — that's the payoff of the quantile work.
